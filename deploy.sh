...
REMOTE_JAR_FILE=gamedoora-config-server.jar
...
pwd
ls
sshpass -p $SSH_PASS scp -o StrictHostKeyChecking=no  ./gamedoora-config-server.service "$SSH_USERNAME@$SSH_HOST:$DEST_PATH"
echo "==========================="
echo "==Stopping service======="
# Stop the service
sshpass -p $SSH_PASS ssh -o StrictHostKeyChecking=no "$SSH_USERNAME@$SSH_HOST" 'sudo systemctl daemon-reload; sudo systemctl stop gamedoora-config-server.service'
echo "========================="
...
sshpass -p $SSH_PASS ssh "$SSH_USERNAME@$SSH_HOST" 'chown -R gamedoora:gamedoora /opt/gamedoora; chmod ug+rwx /opt/gamedoora/*.jar; systemctl restart gamedoora-config-server.service; systemctl is-active --quiet gamedoora-config-server.service && echo Service is running'
echo "========================="
